...
const defs = {
  ...
  compress:{title:"图片压缩",sub:"COMPRESS",icon:"⇣",color:"green",summary:"控制 JPEG / WebP 的导出质量"},
...
const defaults = {
  ...
  format:{format:"png"}, compress:{quality:82}, output:{prefix:"imageflow",expanded:true}
};
const uid = () => "n_" + Math.random().toString(36).slice(2,9);

function makeNode(type,x,y){ return {id:uid(),type,x,y,params:{...defaults[type]}}; }
function seed(){
  state.nodes=[makeNode("input",40,150),makeNode("removebg",365,150),makeNode("resize",690,150),makeNode("format",1015,150),makeNode("output",1340,150)];
...
function summary(n){
  ...
  if(n.type==="compress") return `导出质量 <b>${n.params.quality}%</b>`;
...
function bindNodes(){scene.querySelectorAll(".node").forEach(el=>{
  ...
  el.ondragover=e=>e.preventDefault();el.ondrop=e=>{if(el.dataset.type==="input")loadFiles(e.dataTransfer.files)};
...
function renderInspector(n){if(!n)return;$("#inspector-title").textContent=defs[n.type].title;let html="";
  ...
  if(n.type==="compress")html=range("导出质量","quality",n.params.quality,10,100)+`<div class="hint">质量参数应用于 JPEG 与 WebP；PNG 保持无损。</div>`;
...
function range(label,key,val,min,max){return `<div class="field"><label>${label}<output>${val}</output></label><input type="range" min="${min}" max="${max}" value="${val}" data-param="${key}"></div>`}
