...
import { onFrameContextChanged } from "./batch-cut.js";
...
export function updateButtons() {
  ...
  if (eraserBtn) eraserBtn.disabled = spotOff;
}
...
export function eraserRadius() {
  return Math.max(1, +($("#eraserRadius")?.value || 12));
}

/** 确保当前帧已抠图，供橡皮擦笔触开始时调用。 */
export async function ensureCurrentFrameProcessed() {
  const item = state.items[state.currentFrame];
  if (!item) return null;
  if (!item.processed || !item.resultCanvas) {
    await processItem(item);
  }
  return snapshotFrame(item);
}
 ...
 */
export function brushEraseAtSync(imageX, imageY) {
  const item = state.items[state.currentFrame];
  if (!item?.resultCanvas) return 0;

  const { erased, eraseMask } = brushEraseLocal(
    item.resultCanvas,
    imageX,
    imageY,
    eraserRadius(),
...
/** 结束一次橡皮擦笔触并写入撤销历史。 */
export function commitBrushStroke(before, erasedTotal) {
  if (!before || erasedTotal <= 0) return;
  history.push({
    label: "橡皮擦",
    currentFrame: state.currentFrame,
    frames: [before],
  ...
  updateButtons();
  renderTimeline();
  setStatus(`橡皮擦擦除 ${erasedTotal} 像素 · 半径 ${eraserRadius()}`);
}
...
function finalizeImport(statusMsg) {
...
export async function addFiles(fileList) {
  ...
  finalizeImport(`已导入 ${state.items.length} 张 · 可播放预览或处理抠图`);
}

/**
 * 将批量裁切结果写入帧列表（替换现有帧）。
 * @param {{ name: string, canvas: HTMLCanvasElement, asProcessed?: boolean }[]} frames
 * @param {{ replace?: boolean }} [opts]
 */
export function addCroppedFrames(frames, opts = {}) {
  if (!frames.length) return;
  const replace = opts.replace !== false;
  if (replace) state.items = [];

  for (const frame of frames) {
    const asProcessed = Boolean(frame.asProcessed);
    state.items.push({
      id: crypto.randomUUID(),
      name: frame.name,
  ...
  const kept = frames.filter((f) => f.asProcessed).length;
  finalizeImport(
    kept
      ? `已从精灵图裁入 ${n} 帧（保留透明 ${kept}）· 可继续预览或导出`
      : `已从精灵图裁入 ${n} 帧 · 可播放预览或处理抠图`,
  );
}
