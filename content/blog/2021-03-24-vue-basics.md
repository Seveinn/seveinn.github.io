...
tags:
  []
excerpt: "基本使用 插值、表达式指令、动态属性v html（会有xss风险，会覆盖子组件） computed和watch computed有缓存，data不变则不会重新计算 watch如何深度监听，监听引用类型，拿不到oldVal class和style class: 对象的方式——条件为真时应用变量对应的类名 数组的方式——同…"
status: published
legacyUrl: /posts/2021/03/24/vue-basic.html
---
## 基本使用
